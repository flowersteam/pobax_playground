...
class PlayEnvironment(Environment):
    def __init__(self, tau, pa, pc, gui=False, audio=False):
        ...
        toyname1 = np.array([d - 8.5 for d in self.human_sounds_traj[self.human_sounds[0]][:5]] + [d - 10.25 for d in self.human_sounds_traj[self.human_sounds[0]][5:]]).tostring()
        toyname2 = np.array([d - 8.5 for d in self.human_sounds_traj[self.human_sounds[1]][:5]] + [d - 10.25 for d in self.human_sounds_traj[self.human_sounds[1]][5:]]).tostring()
        toyname3 = np.array([d - 8.5 for d in self.human_sounds_traj[self.human_sounds[2]][:5]] + [d - 10.25 for d in self.human_sounds_traj[self.human_sounds[2]][5:]]).tostring()
        self.toynames = [toyname1, toyname2, toyname3]
        ...
        # GRASP MARGINS
        self.handle_tol = 0.05
        self.object_tol_hand = 0.05
        self.object_tol_tool = 0.05
    ...
    def reset(self):
        ...
        self.reset_toys() # random
        self.reset_tool() # fixed
    ...
    def reset_rand2d(self, region=None):
        if region == 0:
            alpha = np.pi / 4. + np.pi * np.random.random() / 2.
            rdm = np.random.random()
            if rdm < 1. / 3.:
                r = (1. + self.object_tol_hand) * np.random.random()
                return [r * np.cos(alpha), r * np.sin(alpha)]
            elif rdm < 2. / 3.:
                r = (1. + self.object_tol_hand) + ((1.5 + self.object_tol_tool) - (1. + self.object_tol_hand)) * np.random.random()
                return [r * np.cos(alpha), r * np.sin(alpha)]
            else:
                r = (1.5 + self.object_tol_tool) + (2.5 - (1.5 + self.object_tol_tool)) * np.random.random()
                return [r * np.cos(alpha), r * np.sin(alpha)]
        elif region == 1:
            alpha = np.pi / 4. + np.pi * np.random.random() / 2.
            r = (1. + self.object_tol_hand) * np.random.random()
    ...
    def caregiver_moves_obj(self, caregiver_pos, current_toy):
        target = [0., 0.5]
        return [current_toy[0] + self.caregiver_gives_obj_factor * (target[0] - current_toy[0]), current_toy[1] + self.caregiver_gives_obj_factor * (target[1] - current_toy[1]), current_toy[2]]
...
#                 self.toy3 = self.toy3 + self.current_toy3[:2]
        ...
        caregiver_answer = self.get_caregiver_answer(moved_toy or produced_toyname)
        
        if caregiver_answer == "contingent":
            if self.current_toy1[2] >= 1 or events.has_key("voc_toy1"):
                self.caregiver_sound = self.give_label("toy1")
                events["caregiver_voc_toy1"] = True
            elif self.current_toy2[2] >= 1 or events.has_key("voc_toy2"):
                self.caregiver_sound = self.give_label("toy2")
                events["caregiver_voc_toy2"] = True
            elif self.current_toy3[2] >= 1 or events.has_key("voc_toy3"):
                self.caregiver_sound = self.give_label("toy3")
                events["caregiver_voc_toy3"] = True
        elif caregiver_answer == "distractor":
            self.caregiver_sound = self.give_label("distractor")
            events["caregiver_voc_distractor"] = True
        else:
            # No sound
            self.caregiver_sound = [0.]*10 # trajectory that do not move, so won't go into sensorimotor database
        ...
        if len(events.keys()) > 1:
            print "events", self.t, events
        self.events[self.t] = events
        self.t += 1
        if self. t % 100 == 0:
        ...
        # MAP to Delta
#         hand = [hand[0] - hand[0], hand[1] - hand[0], hand[2] - hand[0], hand[3] - hand[0], hand[4] - hand[0],
#                 hand[5] - hand[5], hand[6] - hand[5], hand[7] - hand[5], hand[8] - hand[5], hand[9] - hand[5]]
#           
#         tool = [tool[0] - tool[0], tool[1] - tool[0], tool[2] - tool[0], tool[3] - tool[0], tool[4] - tool[0],
#                 tool[5] - tool[5], tool[6] - tool[5], tool[7] - tool[5], tool[8] - tool[5], tool[9] - tool[5]]
#          
#         toy1 = [toy1[0] - toy1[0], toy1[1] - toy1[0], toy1[2] - toy1[0], toy1[3] - toy1[0], toy1[4] - toy1[0],
#                 toy1[5] - toy1[5], toy1[6] - toy1[5], toy1[7] - toy1[5], toy1[8] - toy1[5], toy1[9] - toy1[5]]
#           
#         toy2 = [toy2[0] - toy2[0], toy2[1] - toy2[0], toy2[2] - toy2[0], toy2[3] - toy2[0], toy2[4] - toy2[0],
#                 toy2[5] - toy2[5], toy2[6] - toy2[5], toy2[7] - toy2[5], toy2[8] - toy2[5], toy2[9] - toy2[5]]
#           
#         toy3 = [toy3[0] - toy3[0], toy3[1] - toy3[0], toy3[2] - toy3[0], toy3[3] - toy3[0], toy3[4] - toy3[0],
#                 toy3[5] - toy3[5], toy3[6] - toy3[5], toy3[7] - toy3[5], toy3[8] - toy3[5], toy3[9] - toy3[5]]
#                 
        
        
        s = context + hand + tool + toy1 + toy2 + toy3 + self_sound + caregiver_sound
